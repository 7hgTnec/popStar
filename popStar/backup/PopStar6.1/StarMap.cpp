...
int map::load(){
	...
	if(fp == NULL){
		HWND hWnd = GetHWnd();
		MessageBox(hWnd,"未发现存档或存档已失效，即将开始新游戏","Load Error", MB_OK); 
		return 0;
	}
	else{
		int ic, ie, is;
		...
		for(int i = 0; i<10; ++i){
			for(int j = 0; j<10; ++j){
				fscanf(fp,"%d %d %d\n", &ic, &ie, &is);
				block[i][j].setColor(ic);
				block[i][j].setExist(ie);
				block[i][j].setSelect(is);
			}
		}
		return 1;
	}
...
int map::save(){
	...
	for(int i = 0; i<10; ++i){
		for(int j = 0; j<10; ++j){
			//fprintf(fp,"1 0 0\n");
			fprintf(fp,"%d %d %d\n", block[i][j].getColor(), block[i][j].isExist(), block[i][j].isSelect());
			//printf("%d %d %d\n", block[i][j].getColor(), block[i][j].isExist(), block[i][j].isSelect());
		}
	}
	fclose(fp);
